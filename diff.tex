...
%DIF DEL sub1/template.tex   Mon Apr  1 13:24:08 2019
%DIF ADD sub2/template.tex   Tue May 28 18:32:57 2019
...
%DIF 171-180c171-181
%DIF < \abstract{A scholarly communication system needs to register, distribute,
%DIF < certify, archive, and incentivize knowledge production. The current
%DIF < article-based system technically fulfills these functions, but
%DIF < suboptimally. I propose a module-based communication infrastructure
%DIF < that attempts to take a wider view of these functions and optimize the
%DIF < fulfillment of the five functions of scholarly
%DIF < communication. Scholarly modules are conceptualized as the constituent
%DIF < parts of a research process as determined by a researcher. These can
%DIF < be text, but also code, data, and any other relevant piece of
%DIF < information. The chronology of these modules is registered by
%DIF -------
\abstract{A scholarly communication system needs to register, %DIF > 
  distribute, certify, archive, and incentivize knowledge %DIF > 
  production. The current article-based system technically fulfills %DIF > 
  these functions, but suboptimally. I propose a module-based %DIF > 
  communication infrastructure that attempts to take a wider view of %DIF > 
  these functions and optimize the fulfillment of the five functions %DIF > 
  of scholarly communication. Scholarly modules are conceptualized as %DIF > 
  the constituent parts of a research process as determined by a %DIF > 
  researcher. These can be text, but also code, data, and any other %DIF > 
  relevant pieces of information that are produced in the research %DIF > 
  process. The chronology of these modules is registered by %DIF > 
%DIF -------
  iteratively linking to each other, creating a provenance record of
%DIF 182-195c183-197
%DIF < parent- and child modules (and a network of modules). These scholarly
%DIF < modules are linked to scholarly profiles, creating a network of
%DIF < profiles, and a network of how profiles relate to their constituent
%DIF < modules. All these scholarly modules would be communicated on the new
%DIF < peer-to-peer Web protocol Dat (https://datproject.org), which provides a
%DIF < decentralized register that is immutable, facilitates greater content
%DIF < integrity than the current system through verification, and is
%DIF < open-by-design. open-by-design would also allow diversity in the way
%DIF < content is consumed, discovered, and evaluated to arise. This initial
%DIF < proposal needs to be refined and developed further based on technical
%DIF < developments of the Dat protocol and its implementations, and
%DIF < discussions within the scholarly community to evaluate the qualities
%DIF < claimed here. Nonetheless, a minimal prototype is available today and
%DIF < this is technically feasible.}
%DIF -------
  parent- and child modules (and a network of modules). These %DIF > 
  scholarly modules are linked to scholarly profiles, creating a %DIF > 
  network of profiles, and a network of how profiles relate to their %DIF > 
  constituent modules. All these scholarly modules would be %DIF > 
  communicated on the new peer-to-peer Web protocol Dat %DIF > 
  (https://datproject.org), which provides a decentralized register %DIF > 
  that is immutable, facilitates greater content integrity than the %DIF > 
  current system through verification, and is %DIF > 
  open-by-design. Open-by-design would also allow diversity in the way %DIF > 
  content is consumed, discovered, and evaluated to arise. This %DIF > 
  initial proposal needs to be refined and developed further based on %DIF > 
  technical developments of the Dat protocol, its implementations, %DIF > 
  and discussions within the scholarly community to evaluate the %DIF > 
  qualities claimed here. Nonetheless, a minimal prototype is %DIF > 
  available today and this is technically feasible.} %DIF > 
%DIF -------
...
communication system can be identified \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{throughout }\DIFaddend the current
...
stages \citep[e.g., methods are evaluated as of higher quality when
  ...
  statistically nonsignificant results;][]{doi:10.1007/bf01173636}\DIFaddbegin \DIFadd{,
leading to what 'natural selection of bad science'
\mbox{%DIFAUXCMD
\citep{doi:10.1098/rsos.160384}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Awareness occurs, but increasingly
\DIFdelbegin \DIFdel{so
for only }\DIFdelend \DIFaddbegin \DIFadd{only for }\DIFaddend those researchers with the financial means to access or make
accessible \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{doi:10.18352/lq.10280}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Restrictions on the sharing
...
suggested micro- and nanopublications \citep{doi:10.7717/peerj-cs.78,
  doi:10.1186/2041-1480-5-28}. \DIFaddbegin \DIFadd{One example of modular, stepwise
...
\caption{\DIFaddFL{An example of modular, stepwise research communication, from the Octopus project (see also }\url{https://perma.cc/TA79-YPH9}\DIFaddFL{).}}
\label{fig:datcom-fig-octo}
\end{figure}

\DIFaddend Modular scholarly outputs, each a separate step in the research
process, could supplement the scholarly article \citep[as detailed
  in][]{doi:10.3390/publications6020021}. Scholarly textbooks \DIFaddbegin \DIFadd{and
monographs }\DIFaddend \citep[i.e., vademecum science;][]{isbn:9780226253251}
...
across findings because they report far after events have happened \DIFaddbegin \DIFadd{and
it is what editors expect}\DIFaddend . Scholarly modules could serve as \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend base
for scholarly articles, reporting more details, less certainty of
findings, and where events are reported closer to their
occurrence. Granular reporting \DIFaddbegin \DIFadd{may help facilitate a shift from
authorship to contributorship \mbox{%DIFAUXCMD
\citep{doi:10.31234/osf.io/dt6e8}}\hspace{0pt}%DIFAUXCMD
, }\DIFaddend could
...
certification \DIFdelbegin \DIFdel{occurs by }\DIFdelend \DIFaddbegin \DIFadd{by peer review is supplemented by }\DIFaddend embedding chronology
...
\citep{doi:10.31219/osf.io/nsv2c}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[see also \url{https://perma.cc/FX7M-H85Y};][]{doi:10.31219/osf.io/nsv2c, doi:10.1038/sdata.2018.221}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Each
...
filesystem and require re-sharing those keys after each change \DIFaddbegin \DIFadd{(at the
protocol level)}\DIFaddend .
...
%DIFDELCMD < {\centering \includegraphics[width=1\linewidth]{Hash_Tree.svg} 
%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{A diagram depicting how a Merkle Tree hashes initial chunks of information into one top hash, with which the content can be verified.}}%DIFAUXCMD
%DIFDELCMD < \label{fig:datcom-fig2}
...
\caption{\DIFaddFL{A diagram conceptually depicting how a Merkle Tree hashes
...
\DIFaddend Any content posted to the Dat protocol is as publicly available as the
...
Alice\DIFdelbegin \DIFdel{her }\DIFdelend \DIFaddbegin \DIFadd{'s }\DIFaddend profile.  The \texttt{follows} property links to other
...
Alice\DIFdelbegin \DIFdel{her }\DIFdelend \DIFaddbegin \DIFadd{'s }\DIFaddend public registrations. \DIFaddbegin \DIFadd{These metadata files may be joined in a
later specification.
}\DIFaddend 
...
\citep[these are examples and modules should not be limited to these
  ...
  outputs;][]{doi:10.1111/j.1467-954X.1990.tb03347.x}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Sometimes we
...
\DIFdelbegin \DIFdel{journal/article based }\DIFdelend \DIFaddbegin \DIFadd{article-based }\DIFaddend system \citep[see
...
is not just a hypothetical exercise, \DIFaddbegin \DIFadd{I developed }\DIFaddend a minimal working
prototype\DIFdelbegin \DIFdel{is
available on the Dat network. This prototype is accessible using
}%DIFDELCMD < \href{https://beakerbrowser.com}{Beaker Browser} %%%
\DIFdel{at
}\texttt{\DIFdel{dat://b06...3dd/}} %DIFAUXCMD
\DIFdel{(see Supporting File for full URL). This
prototype is currently only available }\DIFdelend \DIFaddbegin \DIFadd{. The prototype code supplied below currently only functions
}\DIFaddend within Beaker Browser because specific Application Programmatic
...
for users. By making all content open \DIFaddbegin \DIFadd{by default}\DIFaddend , building on content
\DIFdelbegin \DIFdel{is possible by anyone who feels like it}\DIFdelend \DIFaddbegin \DIFadd{becomes easier}\DIFaddend . For example, someone can build a portal that
...
\DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[e.g.,][]{doi:10.1038/nature.2017.22163}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Stimulating diversity
in how to consume and discover content is key to making scholarly
research accessible to as many people \DIFaddbegin \DIFadd{as possible }\DIFaddend and in order to
...
of \DIFdelbegin \DIFdel{innovative discovery mechanisms }\DIFdelend \DIFaddbegin \DIFadd{an innovative discovery mechanism }\DIFaddend based on openly licensed and
...
approach \DIFaddbegin \DIFadd{than in the scholarly article approach}\DIFaddend , which is
...
digestible than the smaller volume of larger units (i.e., articles)\DIFaddbegin \DIFadd{,
mitigating information onslaught
\mbox{%DIFAUXCMD
\citep{doi:10.1080/1047840X.2012.701161}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
\DIFaddbegin \DIFadd{In the proposed decentralized, modular scholarly communication system
...
scholarly communication \mbox{%DIFAUXCMD
\citep[e.g.,][]{doi:10.1093/biomet/6.1.1} }\hspace{0pt}%DIFAUXCMD
and
should therefore not be excluded by the infrastructure
design. However, some might view this as a limitation.
}

\DIFaddend One of the major points of debate may be that the scholarly modules
...
that the referenced Dat link contains the postdictions, but it \DIFdelbegin \DIFdel{is
}\DIFdelend \DIFaddbegin \DIFadd{might
be }\DIFaddend possible to fake predictions in this manner\DIFdelbegin \DIFdel{(but this is a bigger
problem in the current system)}\DIFdelend . Other mechanisms could
...
are registered. \DIFaddbegin \DIFadd{How to game the proposed system is an active avenue
for further research.
}\DIFaddend 

\DIFaddbegin \DIFadd{The immutability of the Dat protocol that is central to this proposal
...
(e.g., }\texttt{\DIFadd{dat://<hash>@<checksum>+<version>}}\DIFadd{) could help further
reduce this issue.
}

\DIFaddend Despite the potential of building an open-by-design scholarly
...
impractical and premature \DIFaddbegin \DIFadd{(note that no technical limitations prevent
an implementation of the same modular structure on current
technologies, for example GitHub)}\DIFaddend . These developments can occur
...
protocol is currently being built \href{https://github.com/datrs}{in
  Rust} \DIFaddbegin \DIFadd{and }\href{https://github.com/datcxx}{in C++}\DIFaddend , which would
...
more widely used\DIFaddbegin \DIFadd{.
}

\DIFadd{Instead of logging in with passwords, the Dat protocol uses
...
}\url{https://perma.cc/6J9D-ZPAW}\DIFadd{) decentralized security. This
would make the researcher more in control, but also more responsible,
for their operational security}\DIFaddend .
